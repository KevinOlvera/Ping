...
int obtenerDatos(int ds, struct ifreq *ptrInterfaz, unsigned char *My_MAC, unsigned char *My_IP, unsigned char *ownNetMask)
{
    int index = 0;
    ...
    {
        memcpy(My_MAC, ptrInterfaz->ifr_hwaddr.sa_data, 6);
    }
    //Dirección IP
    ...
    {
        memcpy(My_IP, ptrInterfaz->ifr_addr.sa_data + 2, 4);
    }
    //Máscara de red
...
int Default_Interfaz(char *if_name)
{
    int i = 0;
...
int filterICMPreply(unsigned char *trama, unsigned char *destHwAddr, unsigned char *sourceHwAddr, unsigned char *targetIPAddr, unsigned char *sourceIPAddr, int pID, unsigned short seqNumber, int tam_rcv_from, struct timeval *tval_now)
{
    ...
    if (!memcmp(trama + 12, ETHTYPE_ICMP, 2) && !memcmp(trama + 38, identifier, 2) && !memcmp(trama + 41, sqnum, 1))
    {
        ...
        printf("(%d) bytes de %s (%d.%d.%d.%d): icmp_seq=%d ttl=%d time=%.2f ms", tam_rcv_from - 34, Hostname_from_IP(ipstring), trama[26], trama[27], trama[28], trama[29], sqnum[0], trama[22], timeres);
        printf("\n");
